...
{
	// Allocate Open List
	if ((p_open_list = (TAStarNode**)calloc(size, sizeof(TAStarNode*))) == NULL) {
		free(p_open_list);
		p_open_list = NULL;
	...
	// Allocate Close List
	if ((p_close_list = (TAStarNode**)calloc(size, sizeof(TAStarNode*))) == NULL) {
		free(p_close_list);
		p_close_list = NULL;
		return -1;
	}
	...
	int start_col, int end_row, int end_col, TPathNode* &path_list, int& path_length)
{
	...
	TAStarNode* p_parent = NULL;
	TAStarNode* p_child = NULL;
	num_ol_elements = 0;
	num_cl_elements = 0;
	int manhattan_h;
	...
	for (;;) {
		...
		// If the list is empty either we are at the beginning or there is no solution
			...
			if (terrain[id1].p_astar_node == NULL) {  // It is not in a list
		...
		// ***	In this implementation squares that are diagonal from the square
		...
		for (int i = 0; i < 4; i++)
		{
			...
			if (current_row >= 0 && current_row < NUM_ROWS1 && current_col >= 0 && current_col < NUM_COLS1) {
				...
				if (terrain[id2].terrain_cost >= 0) {
					...
					if (terrain[id2].p_astar_node == NULL) {

						// Allocate memory for one element
						if ((p_child = (TAStarNode*)calloc(1, sizeof(TAStarNode))) == NULL) {
							// Return the array passed to the method to initial state
							for (int i = 0; i < NUM_ROWS1 * NUM_COLS1; i++)
								terrain[i].p_astar_node = NULL;
							DeAllocateLists();
		...
		p_open_list[num_ol_elements] = NULL;
...
int CAStar::ComposePath(TAStarNode* p_goal_node)
{
	...
	for (p_temp = p_goal_node; p_temp->p_parent != NULL; p_temp = p_temp->p_parent, shortest_path_length++);

	// Allocate enough memory for the length of the path
	if (p_shortest_path != NULL) {
		free(p_shortest_path);
		p_shortest_path = NULL;
	}
	if ((p_shortest_path = (TPathNode*)calloc(shortest_path_length, sizeof(TPathNode))) == NULL) {
		free(p_shortest_path);
		p_shortest_path = NULL;
		return -1;
	}
